...
def SVM(train_data,
    ...
    # YOUR CODE GOES HERE
    m=svm.SVC(C=1.0,kernel='rbf')
    m.fit(train_data,train_labels)
    train_predictions=m.predict(train_data)
    ...
    # YOUR CODE GOES HERE
    test_predictions=m.predict(test_data)
...
if __name__ == '__main__':
    ...
    # YOUR CODE GOES HERE
    acc=np.mean(accumulated_metrics,axis=0)
    print "%.4f,%.4f,%.4f,%.4f" % (acc[0],acc[1],acc[2],acc[3])
