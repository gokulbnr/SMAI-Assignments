...
def kernelize(X, Y):
    ...
    # YOUR CODE HERE
    kernelX=np.array(X,copy=True)
#    kernelX=np.insert(kernelX,2,X[:,0]*X[:,1],axis=1)
#    kernelX=np.insert(kernelX,2,X[:,0]*X[:,0]*X[:,0]+X[:,1]*X[:,1]*X[:,1],axis=1)
    kernelX=np.insert(kernelX,2,X[:,0]*X[:,0]*X[:,0]*X[:,0]+X[:,1]*X[:,1]*X[:,1]*X[:,1],axis=1)
#    kernelX=np.insert(kernelX,2,X[:,0]*X[:,0]+X[:,1]*X[:,1],axis=1)
#    kernelX=np.insert(kernelX,2,0,axis=1)
    """
    ===========================================================================
    """
